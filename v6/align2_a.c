...
int main()
{
	...
	if (file != NULL)
	{
		fread(&(eStruct.char_1), 1, sizeof(int8_t), file);
		fread(&(eStruct.char_2), 1, sizeof(int8_t), file);
		fread(&(eStruct.uInt_1), 1, sizeof(uint32_t), file);
		fread(&(eStruct.ssInt_2), 1, sizeof(int16_t), file);
		fread(&(eStruct.sInt_3), 1, sizeof(int32_t), file);
		fread(&(eStruct.char_3), 1, sizeof(int8_t), file);
